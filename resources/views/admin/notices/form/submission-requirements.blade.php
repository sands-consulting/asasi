		...
		<tbody>
			<tr v-for="(requirement, index) in submissionRequirements['{{ $type->slug }}']">
				...
				<td>
					...
					<br>
					<input type="hidden" v-bind:name="'submission-requirements[' + index + '][field_required]'" value="0">
					<label>
						<input type="checkbox" v-bind:name="'submission-requirements[' + index + '][field_required]'" v-bind:checked="requirement.field_required" value="1">
						{{ trans('notices.attributes.submission-requirements.required') }}
					</label>
				</td>
				<td><a href="#" class="btn btn-xs btn-default" @click.prevent="deleteSubmissionRequirement('{{ $type->slug }}', index)"><i class="icon-cross2"></i></a></td>
			</tr>
			<tr>
				<td colspan="6" align="center"><a href="#" @click.prevent="addSubmissionRequirement('{{ $type->slug }}')"><i class="icon-plus-circle2"></i> {{ trans('notices.buttons.add-requirement') }}</a></td>
			</tr>
		</tbody>
		@endforeach
	</table>
