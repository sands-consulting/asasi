        ...
        <div class="panel panel-flat">
            ...
            <div class="panel-body">
                {!! Former::open_vertical(route('admin.notices.store-award', $notice->id)) !!}
                    ...
                    <fieldset>
                        <legend><i class="icon-coins"></i> Allocations</legend>
                        <div class="row">
                            @foreach($notice->allocations as $allocation)
                            <div class="col-sm-12">
                                {!! Former::text('allocations[' . $allocation->id . ']') 
                                    ->label($allocation->name) 
                                    ->value($allocation->pivot->amount)!!}
                            </div>
                            @endforeach    
                        </div>
                    </fieldset>
                    ...
                    <fieldset>
                        <legend><i class="icon-watch2"></i> Durations</legend>
                        <div class="row">    
                            <div class="col-sm-12">
                                {!! Former::text('duration') 
                                    ->label('Final Project Duration') !!}
                            </div>
                        </div>
                    </fieldset>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="text-right">
                                <button class="btn btn-default">{{ trans('actions.save') }}</button>
                                <button class="btn btn-primary">{{ trans('actions.publish') }}</button>
                            </div>
                        </div>
                    </div>
