...
<div class="page-title">
    <h4>{{ trans('payment-gateways.title') }}</h4>
</div>
...
<div class="panel panel-flat">
    <div class="panel-body form-datatable-search form-inline">
        <input type="text" name="q[keywords]" class="form-control input-sm" placeholder="{{ trans('payment-gateways.views.index.keywords') }}" v-model="q.keywords">
        <select name="q[status]" class="form-control input-sm" v-model="q.status">
            <option value="" selected="selected">{{ trans('payment-gateways.views.index.status') }}</option>
