...
<ul class="breadcrumb-elements">
    <li>
        <a href="{{ route('admin.users.archives') }}" class="legitRipple">
            <i class="icon-trash"></i> {{ trans('users.views.archives.title') }}
        </a>
    </li>
</ul>
@endsection
...
<div class="panel panel-flat">
    <div class="panel-body form-datatable-search form-inline">
        <input type="text" name="q[keywords]" class="form-control input-sm"
               placeholder="{{ trans('users.views.index.keywords') }}" v-model="q.keywords">
        ...
        <select name="q[status]" class="form-control input-sm" v-model="q.status">
            ...
            @foreach(collect(trans('statuses'))->only('active', 'suspended', 'inactive') as $key => $value)
